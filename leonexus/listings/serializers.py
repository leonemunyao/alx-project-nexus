...
class DealerSerializer(serializers.ModelSerializer):
    ...
    def get_car_count(self, obj):
        ...
        if (
            ...
            and "cars" in obj._prefetched_objects_cache
        ):
            return len(
                [
                    car
                    for car in obj._prefetched_objects_cache["cars"]
                    if car.published
                ]
            )
...
class DealershipCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_specialties(self, value):
        ...
        if not isinstance(value, list):
            raise serializers.ValidationError(
                "Specialties must be a list of strings."
            )
        ...
        for specialty in value:
            if isinstance(specialty, str) and specialty.strip():
                ...
                raise serializers.ValidationError(
                    "Each specialty must be a non-empty string."
                )
    ...
    def update(self, instance, validated_data):
        ...
        if instance.dealer.user != self.context["request"].user:
            raise serializers.ValidationError(
                "You can only update your own dealership."
            )
...
class CarCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    images = CarImageSerializer(many=True, read_only=True)
    # Remove uploaded_images field - we'll handle it in the view
